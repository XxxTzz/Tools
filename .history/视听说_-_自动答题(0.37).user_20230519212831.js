...
// @note         修改说明: 在延迟时间部分加入随机数,以模拟用户操作;修改界面
// ==/UserScript==
...
(function () {
    ...
    // 下方时间你可以根据你的网络情况酌情调整
    // Submit 之后的等待时间
    function submitDelay(MAX = 5061, MIN = 2703) {
        return Math.floor(Math.random() * (MAX - MIN) + MIN)
    }
    // 换页 之后的等待时间
    function pageNextDelay(MAX = 6973, MIN = 3210) {
        return Math.floor(Math.random() * (MAX - MIN) + MIN)
    }
    function inputDelay(MAX = 1279, MIN = 4906) {
        return Math.floor(Math.random() * (MAX - MIN) + MIN)
    }// 输入 之后的等待时间
    function DELAY(MAX = 115979, MIN = 79678) {
        return Math.floor(Math.random() * (MAX - MIN) + MIN)
    }// 等待时间
    ...
    let phrases = ['Yes, he is', 'No, he isn\'t', 'Yes, you are so clever', 'No']
    ...
    async function doTopic() {
        ...
        if ($('img[title="录音"]').length != 0 && config.autodo.includes('auto_luyin')) {
            ...
            await setTixing('视频');
            await doVideo();
            await sleep(config.delay + DELAY()); // 挂机，增加时长
            return true;
        } else {
    ...
    async function initConf() {
        ...
        $('#set_delay').val(config.delay + DELAY());
    }
  ...
  background-color: #fcff6680;
  z-index: 9999;
  border-radius: 10px; /* 添加 border-radius 属性 */
}
    ...
    `).appendTo("head");
        ...
        <div style="clear: both;"></div>
            ...
            <button id="yun_start" style="width: 100%;">START!</button>
            </div>
        `);

        $('#yun_start').click(() => {
            if ($('#yun_start').text() == 'START!') {
